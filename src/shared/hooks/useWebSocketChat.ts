/**
 * ClaudeHydra v4 — WebSocket Chat Hook
 * ======================================
...
export function useWebSocketChat(callbacks: WsCallbacks) {
    ...
    ws.onmessage = (event) => {
      ...
      switch (msg.type) {
        ...
        case 'tool_result':
          ...
          break;
        case 'tool_progress':
          cbs.onToolProgress?.(msg, sid);
          break;
        case 'iteration':
          cbs.onIteration?.(msg, sid);
          break;
        case 'complete':
        ...
        case 'pong':
          ...
          break;
        case 'heartbeat':
          // Server heartbeat during long tool execution — reset timers
          break;
      }
    };
  ...
  const sendExecute = useCallback((prompt: string, model?: string, toolsEnabled?: boolean, sessionId?: string) => {
    const ws = wsRef.current;
    if (!ws || ws.readyState !== WebSocket.OPEN) return;

    streamingSessionIdRef.current = sessionId ?? null;
