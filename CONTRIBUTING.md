...
# 3. Backend
cp backend/.env.example backend/.env   # Fill in API keys (ANTHROPIC_API_KEY, GOOGLE_API_KEY)
cd backend
...
Example:
```bash
git commit -m "feat: add cross-session visibility with GeminiHydra"
git commit -m "fix: handle WebSocket heartbeat during streaming"
```
...
### Backend
Rust · Axum 0.8 · SQLx · PostgreSQL 17 · tower-http · tower_governor
...
├── handlers.rs          # Route handlers (chat, models, OAuth, coordination)
├── state.rs             # AppState struct
├── model_registry.rs    # Dynamic model cache (Anthropic Claude, Google Gemini)
...
```env
# Database
DATABASE_URL=postgresql://postgres:password@localhost:5433/claudehydra
...
OAUTH_REDIRECT_URI=http://localhost:8082/api/auth/callback
...
- **Multi-Agent Coordination** — Orchestrate workflows with Commander, Coordinator, and Executor roles
- **Chat Streaming** — Real-time responses via WebSocket
- **OAuth Integration** — Anthropic Claude MAX Plan for flat-rate API access
- **Cross-Session Visibility** — Read-only view of GeminiHydra sessions
...
- **Database**: Fly.io Postgres cluster (`claudehydra_v4_backend` database)

## Notes

- **WebSocket Streaming**: Frontend heartbeat MUST pause during tool execution to avoid timeout (see Troubleshooting).
...
- **Cross-Session**: Partner client proxies to GeminiHydra backend (port 8081).
...
**WebSocket disconnects during chat streaming?**
...
**Port conflicts?**
```bash
# Check which process uses port 5199 or 8082
